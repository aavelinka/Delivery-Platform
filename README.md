...
- outbox pattern для публикации событий в Kafka в `order-service`,
  `courier-service` и `tracking-service`.
...
- Redis: поднят в Compose, но бизнес-логика на него пока не завязана;
- MongoDB: в текущем коде не используется;
- OpenTelemetry: в текущем коде не используется.
...
Сейчас использует PostgreSQL. MongoDB может быть рассмотрен позже, если
понадобится хранить очень большой поток геособытий как документы.
...
Пока не реализован. Планируется после завершения базового MVP.
...
Сервис поднят в `docker-compose.yml`, но в текущей кодовой базе еще не
используется для кеша, rate limiting, locks или idempotency.
...
стандартную переменную окружения сервиса, например `AUTH_DATABASE_URL` или
`ORDER_DATABASE_URL`.
...
## Что Нужно Доделать До Publish-Ready Состояния

- выбрать и зафиксировать лицензию;
- решить, нужен ли Redis в runtime сейчас, либо убрать его из текущего стека;
- решить, остается ли `tracking-service` на PostgreSQL или действительно
  переводится на MongoDB в следующем этапе.
