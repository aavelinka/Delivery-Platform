...
- `admin-service` с admin-only overview и cross-service health/summary агрегацией;
- единый `x-request-id` и structured HTTP request logging во всех сервисах;
- lightweight distributed tracing через `traceparent`, `x-trace-id` и
  `metadata.trace` в Kafka events/outbox;
...
из других сервисов по HTTP без прямого доступа к их базам данных.
...
- расширить `admin-service` более детальной business analytics поверх summary endpoints.
- добавить полноценный OpenTelemetry export поверх текущего lightweight tracing.
